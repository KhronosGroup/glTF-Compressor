...
import { ImagePreviewRenderer } from './image_preview_renderer.js';
...
import previewShader from './shaders/preview.frag';
...
class gltfRenderer
{
    constructor(context)
    {
        ...
        shaderSources.set("preview.frag", previewShader);
    ...
    init(state)
    {
        ...
        if (!this.initialized){
            ...
            this.imagePreviewRenderer = new ImagePreviewRenderer(this.webGl);
        }
        else {
    ...
    drawScene(state, scene)
    {
        ...
        this.webGl.context.enable(this.webGl.context.SCISSOR_TEST);
        const scissorRects = [
            [0,0, state.compressorParameters.sliderPosition * this.currentWidth, this.currentHeight],
            [state.compressorParameters.sliderPosition * this.currentWidth, 0, this.currentWidth, this.currentHeight]
        ];
        const scissorRect = scissorRects[state.compressorParameters.previewCompressed? 1 : 0];
        this.webGl.context.scissor(scissorRect[0], scissorRect[1], scissorRect[2], scissorRect[3]);
        ...
        this.webGl.context.disable(this.webGl.context.SCISSOR_TEST);
    }
    ...
    drawPrimitive(state, renderpassConfiguration, primitive, node, viewProjectionMatrix, transmissionSampleTexture)
    {
        ...
        for (; textureIndex < material.textures.length; ++textureIndex)
        {
            ...
            if (!this.webGl.setTexture(location, state.gltf, info, textureIndex, state.compressorParameters.previewCompressed)) // binds texture and sampler
            {
    ...
    drawPreviewImage(state, scene)
    {
        ...
        const fragDefines = [];
        this.pushFragParameterDefines(fragDefines, state);
        this.imagePreviewRenderer.drawImagePreview(this.webGl, state.compressorParameters.previewTextureIndex, state, this.shaderCache, fragDefines);
