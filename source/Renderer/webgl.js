...
class gltfWebGl
{
    ...
    setTexture(loc, gltf, textureInfo, texSlot, bindCompressed = false)
    {
        ...
        // GSV-KTX
        // mark the compressed image texture for update
        if(image.compressedTextureNeedUpdate)
        {
            gltfTex.compressedNeedUpdate = true;
            // clear the compressed image texture before updating
            if(gltfTex.compressedGLTexture !== undefined)
            {
                this.context.deleteTexture(gltfTex.compressedGLTexture);
                gltfTex.compressedGLTexture = undefined;
            }
            image.compressedTextureNeedUpdate = false;
        }
        ...
        if (gltfTex.compressedGLTexture === undefined)
        {
            if (image.compressedMimeType === ImageMimeType.KTX2 ||
                image.compressedMimeType === ImageMimeType.GLTEXTURE)
            {
                // these image resources are directly loaded to a GPU resource by resource loader
                gltfTex.compressedGLTexture = image.compressedImage;
            }
            else
            {
                // other images will be uploaded in a later step
        ...
        if (!bindCompressed && !gltfTex.initialized)
        {
        ...
        if (bindCompressed && gltfTex.compressedNeedUpdate)
        {
            const gltfSampler = gltf.samplers[gltfTex.sampler];
            ...
            // upload images that are not directly loaded as GPU resource
            if (image.compressedMimeType === ImageMimeType.PNG ||
                image.compressedMimeType === ImageMimeType.JPEG ||
                image.compressedMimeType === ImageMimeType.WEBP ||
                image.compressedMimeType === ImageMimeType.HDR)
            {
                // the check `GL.SRGB8_ALPHA8 === undefined` is needed as at the moment node-gles does not define the full format enum
                const internalformat = (textureInfo.linear || GL.SRGB8_ALPHA8 === undefined) ? GL.RGBA : GL.SRGB8_ALPHA8;
                this.context.texImage2D(image.type, image.miplevel, internalformat, GL.RGBA, GL.UNSIGNED_BYTE, image.compressedImage);
            }
            ...
            gltfTex.compressedNeedUpdate = false;
            image.compressedTextureNeedUpdate = false;
        }

        return bindCompressed? !gltfTex.compressedNeedUpdate : gltfTex.initialized;
    }
